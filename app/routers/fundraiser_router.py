...
from ..schemas.fundraiser_schema import FundraiserCreate, UploadImage
...
# -----------------------------------------------------------------
# 2. SEPARATE IMAGE UPLOAD (Optional / Legacy)
# -----------------------------------------------------------------
@router.post("/upload/image/{fundraiser_id}")
async def upload_image(fundraiser_id: int, images: UploadImage, db: Session = Depends(get_db)):
    """
    Uploads images to an existing campaign.
    """
    fundraiser = db.query(FundraiserMaster).filter(FundraiserMaster.fundraiser_id == fundraiser_id).first()
    
    if not fundraiser:
        raise HTTPException(status_code=404, detail="Fundraiser ID not found")

    try:
        provided_images = images.model_dump(exclude_none=True)
        
        for field, base64_data in provided_images.items():
            if base64_data and base64_data.startswith("data:"):
                upload_result = cloudinary.uploader.upload(base64_data)
                setattr(fundraiser, field, upload_result["secure_url"])

        db.commit()
        return {"message": "Images processed successfully", "id": fundraiser_id}
